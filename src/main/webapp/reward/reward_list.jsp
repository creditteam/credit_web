								...
								<table class="table table-hover">
									...
									<c:forEach items="${pd.data}" var="reward">
										<tr>
											...
											<td>${reward.province } ${reward.city}</td>
											<td><c:if test="${reward.rewardAmount eq '3000'}">${reward.rewardAmount } (元以上)</c:if>
					   							<c:if test="${reward.rewardAmount ne '3000'}">${reward.rewardAmount } (元)</c:if></td>
											<td><span class="label label-warning">
